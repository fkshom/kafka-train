...
rm -f *.keytab *.crt *.csr *_creds *.jks *.pem *.req *.srl
...
# Generate CA key
openssl req -new -x509 -keyout snakeoil-ca-1.key -out snakeoil-ca-1.crt -days 365 -subj '/CN=localhost/OU=TEST/O=CONFLUENT/L=PaloAlto/S=Ca/C=US' -passin pass:confluent -passout pass:confluent
...
i=kafka
	...
	keytool -keystore kafka.keystore.jks -alias $i -certreq -file $i.csr -storepass confluent -keypass confluent
	...
	keytool -keystore kafka.keystore.jks -alias CARoot -import -file snakeoil-ca-1.crt -storepass confluent -keypass confluent -noprompt

	keytool -keystore kafka.keystore.jks -alias $i -import -file $i-ca1-signed.crt -storepass confluent -keypass confluent -noprompt

	# Create truststore and import the CA cert.
	keytool -keystore kafka.truststore.jks -alias CARoot -import -file snakeoil-ca-1.crt -storepass confluent -keypass confluent -noprompt
