...
# create CA
openssl req -new -x509 -keyout ca-key -out ca-cert -days $VALIDITY -subj "/C=US/ST=California/L=San Diego/O=Development/OU=Dev/CN=example.com" -passin pass:$PASSWORD -passout pass:$PASSWORD
...
# Create server cert
keytool -genkey -keystore kafka.server.keystore.jks -alias localhost -validity $VALIDITY -keyalg RSA \
   -dname "CN=localhost, OU=TEST, O=CONFLUENT, L=PaloAlto, S=Ca, C=US" -storepass $PASSWORD -keypass $PASSWORD -noprompt
...
# Create client cert
keytool -genkey -keystore kafka.client.keystore.jks -alias localhost -validity $VALIDITY -keyalg RSA \
   -dname "CN=localhost, OU=TEST, O=CONFLUENT, L=PaloAlto, S=Ca, C=US" -storepass $PASSWORD -keypass $PASSWORD -noprompt
